...
In a simple linear regression model, a single response measurement Y is related to a single predictor  X for each observation. The critical assumption of the model is that the conditional mean function is linear: $E(Y |X) = \alpha + \beta X$. In most problems, more than one predictor variable will be available. 
...
8. Chesher A, Irish M. Residual analysis in the grouped and censored normal linear model[J]. Journal of Econometrics, 1987, 34(1鈥?2):33-61.
